...
@pytest.fixture
def mock_cifar10():
    with patch("src.data.cifar10.datasets.CIFAR10", side_effect=_fake_cifar10) as mock:
        yield mock


class TestGetCifar10Dataloaders:
    ...
    def test_label_dtype(self, mock_cifar10):
        train_loader, _ = get_cifar10_dataloaders(batch_size=16)
    ...
    def test_custom_num_workers(self, mock_cifar10):
        train_loader, test_loader = get_cifar10_dataloaders(num_workers=4)
...
class TestAugmentation:
    ...
    def test_random_crop_padding(self, mock_cifar10):
        get_cifar10_dataloaders(use_augmentation=True)
        train_call = mock_cifar10.call_args_list[0]
        crop = train_call.kwargs["transform"].transforms[1]
...
class TestNormalization:
    def test_normalize_mean(self, mock_cifar10):
        get_cifar10_dataloaders()
        train_call = mock_cifar10.call_args_list[0]
        normalize = train_call.kwargs["transform"].transforms[-1]
